...
```{r}
horseshoe <- stan(file = 'Horseshoe.stan', data = diabetes_dat,chains=1,control = list(adapt_delta = 0.99),init=0,seed=1)
```



```{r}
summary(horseshoe,pars=c('beta','sigma'))
params=c('beta[1]','beta[7]','beta[12]','beta[21]','beta[34]','beta[47]','beta[56]','beta[64]','sigma')
traceplot(horseshoe,pars=params)
```

```{r}
Xte = as.matrix(X_test,nrow=100,ncol=64)
draws = extract(horseshoe,pars=c('beta','sigma'))
beta_h = apply(draws$beta,2,mean)
y_pred = Xte %*% beta_h;
mean((y_test$V1 - y_pred)**2)
```

Similar to Lasso but without need to optimise and with the capacity to handle prediction uncertainty. 

Alternative specificiations may be explored such as the ['Finnish' horseshoe](https://arxiv.org/abs/1610.05559) 
